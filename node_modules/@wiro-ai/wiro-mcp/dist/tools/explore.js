...
export function registerExplore(server, client) {
    server.registerTool('explore', {
        ...
        try {
            ...
            if (!result.result || !result.explore?.length) {
                return {
                    content: [{ type: 'text', text: 'No explore data available.' }],
...
function formatExplore(categories) {
    const lines = ['## Explore Models'];
    lines.push('');
