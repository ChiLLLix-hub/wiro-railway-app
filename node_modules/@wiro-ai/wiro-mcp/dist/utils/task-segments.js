...
export function normalizeTaskRawContent(content, options = {}) {
    if (!isRecord(content)) {
        return {
            hasSegments: false,
            segments: [],
...
export function isToolCallSegment(segment) {
    return segment.type === 'function_call'
        || segment.type === 'custom_tool_call';
}
function normalizeTaskSegment(value) {
    if (!isRecord(value))
        return null;
    ...
    if (value['type'] === 'function_call') {
        ...
        const argumentsValue = boundedString(value['arguments'], MAX_TOOL_INPUT_BYTES);
        if (!common || argumentsValue === null)
            return null;
        return {
            type: 'function_call',
            ...common,
...
function normalizeToolCallFields(value) {
    ...
    const status = typeof value['status'] === 'string'
        && TOOL_CALL_STATUSES.has(value['status'])
        ? value['status']
        : null;
...
function normalizeTaskUsage(value) {
    ...
    const inputDetails = normalizeUsageCounters(value['input_tokens_details'], INPUT_USAGE_DETAIL_KEYS);
    const outputDetails = normalizeUsageCounters(value['output_tokens_details'], OUTPUT_USAGE_DETAIL_KEYS);
    const serverToolUse = normalizeUsageCounters(value['server_tool_use'], SERVER_TOOL_USAGE_KEYS);
...
function normalizeUsageCounters(value, allowedKeys) {
...
function normalizeUsageCounter(value) {
...
//# sourceMappingURL=task-segments.js.map